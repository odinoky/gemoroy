...
const { smsg, isUrl, generateMessageTag, getBuffer, getSizeMedia, fetchJson, await, sleep } = require('../lib/exif')
...
async function startichi() {
    const ichi = ichiConnect({
        ...
        browser: ['Sabo Multi Device Ya Ges Ya','Safari','1.0.0'],
        auth: state
    })
    ...
    ichi.ev.on('messages.upsert', async chatUpdate => {
        ...
        require("./handler")(ichi, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
  ...
  console.log(err)
    ...
    ichi.ev.on('connection.update', async (update) => {
        ...
        let reason = new Boom(lastDisconnect?.error)?.output.statusCode
            ...
            else if (reason === DisconnectReason.connectionClosed) { console.log("Connection closed, reconnecting...."); startichi(); }
            else if (reason === DisconnectReason.connectionLost) { console.log("Connection Lost from Server, reconnecting..."); startichi(); }
            ...
            else if (reason === DisconnectReason.restartRequired) { console.log("Restart Required, Restarting..."); startichi(); }
            else if (reason === DisconnectReason.timedOut) { console.log("Connection TimedOut, Reconnecting..."); startichi(); }
    ...
    // Add Other
      ...
      * @param {*} options
      */
     ichi.sendFileUrl = async (jid, url, caption, quoted, options = {}) => {
      let mime = '';
    ...
    /** Send List Messaage
      ...
      *@param {*} quoted
      */
        ichi.sendListMsg = (jid, text = '', footer = '', title = '' , butText = '', sects = [], quoted) => {
        let sections = sects
        var listMes = {
        text: text,
        footer: footer,
     ...
     * @returns 
     */
        ichi.send5ButMsg = (jid, text = '' , footer = '', but = []) =>{
     ...
     * @returns
     */
    ichi.send5ButVid = async (jid , text = '' , footer = '', vid, but = [], options = {}) =>{
     ...
     * @returns
     */
    ichi.send5ButGif = async (jid , text = '' , footer = '', gif, but = [], options = {}) =>{
