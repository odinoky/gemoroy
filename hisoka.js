...
module.exports = hisoka = async (hisoka, m, chatUpdate) => {
    try {
        ...
        let groupAdmins = m.isGroup ? await GroupAdmins(groupMembers) : ''
        let isBotAdmins = m.isGroup ? isBotAdmins.find(a => a.jid == hisoka.user.jid).isAdmin : false
        let isAdmin = m.isGroup ? isAdmin.find(a => a.jid == m.sender).isAdmin : false
